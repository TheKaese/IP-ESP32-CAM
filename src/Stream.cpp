    ...
    for (;;)
    {
        ...
        else
        {
            ...
            if (fb->format != PIXFORMAT_JPEG)
            {
                ...
                if (!jpeg_converted)
                {
                    Logger.log("JPEG compression failed");
                }
            }
            else
            {
...
void handleJPGSstream(void)
{
    Logger.log("Handle stream request");
...
void startStreamServer()
{
    ...
    Logger.log("Starting stream server on port: '%d'\n", streamPort);
    ...
    Logger.log("Camera Stream Ready!  Use '%s' to connect", streamURL);
}
